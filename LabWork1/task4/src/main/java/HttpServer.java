...
class HttpServer {
    ...
    // Taken from https://www.urldecoder.io/java/
    ...
    private void process(Request request, Response response) throws IOException {
        ...
        String uri = decodeValue(request.getUri());
        if ((!uri.endsWith(".png") && !uri.endsWith(".html") && !uri.endsWith(".htm"))) {
            uri += "/index.html";
        }
        ...
        if (file.isFile()) {
            ...
            String hash = Long.toString(file.lastModified() + file.length());

            // response.setHeader("ETag", );
            if (request.getHeader("If-None-Match") != null &&
                    request.getHeader("If-None-Match").equals(hash)) {
